...
impl DbBkp {
    ...
    /// Backup with progress tracking via an atomic byte counter.
    /// The `bytes_written` counter is updated as compressed data is written to storage.
    pub async fn backup_with_progress(
        ...
        let writer = self.storage_provider.create_writer(&name).await?;
        let progress_writer = ProgressWriter::new(writer, bytes_written);
        let mut compressed_writer = Compressor::new(
            progress_writer,
            compression_format,
            Compression::new(compression_level),
        );
        ...
        self.database_connection
            .connection
            .backup(&mut compressed_writer)
            .await?;

        let progress_writer = compressed_writer.finish()?;
